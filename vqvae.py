...
class ReZero(HelperModule):
    def build(self, in_channels: int, res_channels: int):
        self.layers = nn.Sequential(
            nn.Conv2d(in_channels, res_channels, 3, stride=1, padding=1, bias=False,padding_mode='zeros'),
            nn.BatchNorm2d(res_channels),
            nn.ReLU(inplace=True),

            nn.Conv2d(res_channels, in_channels, 3, stride=1, padding=1, bias=False,padding_mode='zeros'),
...
class ResidualStack(HelperModule):
    def build(self, in_channels: int, res_channels: int, nb_layers: int, use_ffc: bool = False):
        if use_ffc:
            self.stack = nn.Sequential(*[FFCResnetBlock(in_channels, padding_type='zeros',
                                         norm_layer=nn.BatchNorm2d,
                                         activation_layer=nn.ReLU,ratio_gin=0.75, ratio_gout=0.75,
                                         inline=True) 
    ...
    def build(self, 
        ...
        for _ in range(downscale_steps):
            layers.append(nn.Sequential(
                nn.Conv2d(c_channel, n_channel, 4, stride=2, padding=1,padding_mode='zeros'),
                nn.BatchNorm2d(n_channel),
                nn.ReLU(inplace=True),
            ))
            c_channel, n_channel = n_channel, hidden_channels
        layers.append(nn.Conv2d(c_channel, n_channel, 3, stride=1, padding=1,padding_mode='zeros'))
...
class Decoder(HelperModule):
    def build(self, 
        ...
        layers = [nn.Conv2d(in_channels, hidden_channels, 3, stride=1, padding=1,padding_mode='zeros')]
...
class CodeLayer(HelperModule):
    def build(self, in_channels: int, embed_dim: int, nb_entries: int):
        self.conv_in = nn.Conv2d(in_channels, embed_dim, 1,padding_mode='zeros')
    ...
    def forward(self, x: torch.FloatTensor) -> Tuple[torch.FloatTensor, float, torch.LongTensor]:
        ...
        embed_dist, embed_ind = dist.min(1)
