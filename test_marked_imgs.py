...
if __name__ == '__main__':
    ...
    parser.add_argument('--root_path', type=str, default='D:/dataset/FMD/OriCroppedMarked')
    ...
    test_dataset = AnomalyDataset(args.root_path, cfg=cfg, mode='test',transform=transform)
    test_loader = torch.utils.data.DataLoader(test_dataset,batch_size=1,
                                               num_workers=nw)
    for i,(img,name) in tqdm.tqdm(enumerate(test_loader)):
        print('img shape 3: ',img.shape)
        savedir = Path(name[0]).parent.parent / 'Output'
        imgname = Path(name[0]).name[:-4]
        ...
        save_image(out,savedir/f'{imgname}_out.jpg', normalize=True, value_range=(-1,1))
        res_score = torch.abs(img-out)
        save_image(res_score,savedir/f'{imgname}_res.jpg', normalize=True, value_range=(0,1))
        ssim_score = 1 - ssim(img,out)
        save_image(ssim_score,savedir/f'{imgname}_ssim.jpg', normalize=True, value_range=(0,1))
        gms_score = 1 - gms(img,out)
        save_image(gms_score,savedir/f'{imgname}_gms.jpg', normalize=True, value_range=(0,1))
